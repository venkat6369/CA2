...
sigma(ht_mlr1)/ mean(testing_data$Target)
#0.718 which is a high value , value the lower the better, This model is not the best model to predict heart attack. 
#--------------------------------------------
library("MASS")
...
sigma(ht_model_2_test)/ mean(testing_data$Target)
...
#inputting some data
ht_model_data_OP<- data.frame(Age = c(41),
                           Sex = c(0),  
...
#prediction
predicted_heart_attack <- predict(ht_model_2, ht_model_data_OP)
